...
EC_CPU_NOT_SUPPORT_INSTRUCTION_SET = OBDErrorCodeTemplate(2014, "{server}'s cpu does not support {instruction_set}")
...
EC_OCP_SERVER_JAVA_VERSION_ERROR = OBDErrorCodeTemplate(4359, "{server}: ocp-server need java with version {version}")
...
SUG_OCP_EXPRESS_REDUCE_MEM = OBDErrorSuggestionTemplate('Please reduce the `memory_size` or execute `echo 1 > /proc/sys/vm/drop_caches` as root to release cached.', fix_eval=[FixEval(FixEval.DEL, 'memory_size')])
...
SUG_OCP_SERVER_REDUCE_MEM = OBDErrorSuggestionTemplate('Please reduce the `memory_size` or execute `echo 1 > /proc/sys/vm/drop_caches` as root to release cached.', fix_eval=[FixEval(FixEval.DEL, 'memory_size')])
