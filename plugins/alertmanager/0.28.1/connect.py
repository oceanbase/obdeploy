...
class AlertmanagerAPICursor(object):
    ...
    def _request(self, method, api, data=None, stdio=None):
        ...
        if return_code == 200:
            return True
        stdio.verbose("request alertmanager failed: %s" % content)
        return False
...
def connect(plugin_context, target_server=None, *args, **kwargs):
    ...
    if target_server:
        ...
        stdio.start_loading('Connect to Alertmanager ({})'.format(target_server))
    else:
        servers = cluster_config.servers
        stdio.start_loading('Connect to Alertmanager')
    ...
    while count and servers:
        ...
        for server in servers:
            ...
            username = new_username if new_config and count % 2 else username
            ...
            if config.get('web_config', {}).get('tls_server_config'):
                if config['web_config']['tls_server_config'] and (config['web_config']['tls_server_config'].get('cert_file') or config['web_config']['tls_server_config'].get('cert')):
                    ssl = True
            stdio.verbose('connect alertmanaget ({}:{} by user {})'.format(server.ip, config['port'], username))
            api_cursor = AlertmanagerAPICursor(ip=server.ip, port=config['port'], username=username, password=password, ssl=ssl)
