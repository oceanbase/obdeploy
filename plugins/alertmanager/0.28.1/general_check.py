...
def general_check(plugin_context, init_check_status=False, work_dir_check=False, work_dir_empty_check=True, precheck=False, *args, **kwargs):
    ...
    stdio.start_loading('Check before start alertmanager')
    if len(cluster_config.servers) > 1:
        critical(cluster_config.servers[0], 'server number', 'alertmanager only support single server.')
    ...
    for server in cluster_config.servers:
        ...
        if server_config.get('receivers'):
            for receiver in server_config.get('receivers'):
                if not cluster_receivers_conf.get(receiver):
                    critical(server, 'config', f"The notification configuration of {receiver} must be set")
        ...
        if work_dir_check:
            ...
            if not server_config.get('log_dir'):
                server_config['log_dir'] = '%s/log' % home_path
