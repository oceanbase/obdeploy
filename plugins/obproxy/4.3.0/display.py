...
def display(plugin_context, cursor, config_encrypted, display_encrypt_password='******', *args, **kwargs):
    ...
    for server in servers:
        ...
        if res:
            ...
            continue
        result.append(data)
    stdio.print_list(result, ['ip', 'port', 'prometheus_port', 'rpc_listen_port', 'status'],
        lambda x: [x['ip'], x['listen_port'], x['prometheus_listen_port'], x['rpc_listen_port'], x['status']], title=cluster_config.name)
    server = servers[0]
    with_observer = False
    server_config = cluster_config.get_server_conf(server)
    if server_config.get('dns'):
        ip = server_config['dns']
        port = server_config['listen_port']
    elif (server_config.get('vip_address') and server_config.get('vip_port')):
        ip = server_config['vip_address']
        port = server_config['vip_port']
    else:
        ip = server.ip
        port = server_config['listen_port']
    ...
    for comp in ['oceanbase', 'oceanbase-ce']:
        if comp in cluster_config.depends:
            ...
            cmd = 'obclient -h%s -P%s -u%s %s-Doceanbase -A \n' % (ip, port, user, '-p%s ' % ((passwd_format(password) if password else '') if not display_encrypt_password else passwd_format(display_encrypt_password)))
            break
