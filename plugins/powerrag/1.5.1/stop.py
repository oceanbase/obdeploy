...
def stop(plugin_context, *args, **kwargs):
    ...
    stdio.start_loading('Stop powerrag')
        ...
        if services_status[server] or service_names:
            with EnvVariables(env_map, client):
                ...
                for service in stop_services:
                    info = services_status[server][service]
                    if info['state'] == 'exited':
                        continue
                    else:
                        stop_cmd = cmd_prefix + f"{dc_prefix} docker compose --project-name {compose_project} -f docker-compose.yaml --env-file .env stop {service}"
                        ret = client.execute_command(stop_cmd)
                        if not ret:
                            stdio.stop_loading('fail')
                            stdio.error('%s stop failed: %s' % (server, service))
