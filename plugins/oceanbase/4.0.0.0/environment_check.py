...
def environment_check(plugin_context, work_dir_empty_check=True, generate_configs={}, *args, **kwargs):
    ...
    ip_servers = plugin_context.get_variable('ip_servers')
    ...
    for server in cluster_config.servers:
        ...
        if not client.execute_command('lscpu | grep Flags | grep avx'):
            critical(server, 'cpu', err.EC_CPU_NOT_SUPPORT_AVX.format(server=server), [])
