...
def construct_opts(server_config, param_list, rs_list_opt, cfg_url, cmd, need_bootstrap, home_path=None):
    ...
    # shared_storage_info is ObD-meta consumed only by bootstrap.py
    # (ALTER SYSTEM BOOTSTRAP ... SHARED_STORAGE_INFO). Do not forward it
    # to observer via -o, or observer will reject the unknown parameter.
    not_cmd_opt = [
...
def start_pre(plugin_context, *args, **kwargs):
    ...
    # oceanbase.ai + oblogservice gate: same as start_check_pre / bootstrap.
    # When both conditions hold, enable_logservice must be forwarded to the
    # observer command line so the started observer negotiates with the
    # external logservice. We inject it into server_config below so the
    # existing "-o key=value" flow picks it up regardless of whether the
    # user set it at global or per-server scope in yaml.
    global_conf = cluster_config.get_global_conf()
    ai_logservice_enabled = (
        cluster_config.name == const.COMP_OB_AI
        and bool(global_conf.get('enable_logservice'))
    )
    ...
    if os.path.exists(default_parameters_json):
        with FileUtil.open(default_parameters_json, 'rb') as f:
            optimize_data = json.load(f)
    else:
        stdio.verbose('default_parameter.json not found, no optimize ')

    def _optimize():
        for default_parameters in optimize_data:
            if default_parameters['scenario'] == scenario:
                if 'parameters' in default_parameters:
                    for parameter in default_parameters['parameters']['cluster']:
                        stdio.verbose('update %s to %s because of scenario' % (parameter['name'], parameter['value']))
                        if not parameter['name'] in server_config:
                            server_config[parameter['name']] = parameter['value']
    ...
    for server in cluster_config.servers:
        ...
        # Guarantee enable_logservice=true reaches observer for oceanbase.ai +
        # oblogservice deploys. Global switch is the source of truth; if the
        # per-server dict didn't already carry it (e.g. user set it globally
        # instead of per-server, or the reload flow filtered it out), inject
        # it here so construct_opts emits `-o enable_logservice=True`.
        if ai_logservice_enabled:
            server_config['enable_logservice'] = True
            if start_parameters is not server_config:
                start_parameters['enable_logservice'] = True
        ...
        cmd = []
        if use_parameter:
            optimize_scenario = ['express_oltp', 'complex_oltp', 'olap', 'htap', 'kv']
            if scenario in optimize_scenario:
                _optimize()
