...
# Keep in sync with oblogservice_util.should_skip_bootstrap so that
# shared_storage_info is validated the same way as oblogservice's
# object_store_url. Any of these placeholders means the user left the
# template unchanged.
_OBJECT_STORE_URL_PLACEHOLDERS = (
    'your-bucket',
    'your-object-storage-host',
    'access_id=xxx',
    'access_key=yyy',
)


def _validate_ai_logservice_config(cluster_config, stdio):
    """Validate oceanbase.ai + enable_logservice specific configs.

    Mirrors the non-empty + placeholder check that oblogservice applies to
    object_store_url. Only runs when the gate (component == oceanbase.ai
    and enable_logservice == true) is satisfied; other deployments are
    ...
    if not shared_storage_info:
        stdio.error(
            'oceanbase.ai start check: shared_storage_info is required when '
            'enable_logservice is true. Set it under the oceanbase.ai global config.'
        )
        return False
    for placeholder in _OBJECT_STORE_URL_PLACEHOLDERS:
        if placeholder in shared_storage_info:
            stdio.error(
                "oceanbase.ai start check: shared_storage_info still contains the "
                "template placeholder '%s'. Replace it with the real object "
                "storage URL before deploying." % placeholder
            )
            return False
    return True
...
def start_check_pre(plugin_context, init_check_status=False, strict_check=False, work_dir_check=False, work_dir_empty_check=True, generate_configs={}, precheck=False, source_option='start', *args, **kwargs):
    ...
    # oceanbase.ai + oblogservice pre-check.
    # Gate: component is oceanbase.ai AND enable_logservice is true. Any
    # other combination goes through the original start_check_pre path
    # without modification.
    if cluster_config.name == const.COMP_OB_AI and cluster_config.get_global_conf().get('enable_logservice'):
        if not _validate_ai_logservice_config(cluster_config, stdio):
