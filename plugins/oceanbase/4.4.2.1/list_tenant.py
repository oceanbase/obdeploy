...
def list_tenant(plugin_context, cursor, config_encrypted, relation_tenants={}, *args, **kwargs):
    ...
    if tenant_infos:
        comp_name = plugin_context.cluster_config.name
        show_protection = comp_name == COMP_OB
        headers = ['tenant_name', 'tenant_type', 'compatibility_mode', 'primary_zone', 'max_cpu',
                   'min_cpu', 'memory_size', 'max_iops', 'min_iops', 'log_disk_size',
                   'iops_weight', 'tenant_role']
        if show_protection:
            headers.extend(['protection_mode', 'protection_level'])

        def tenant_basic_row(x):
            row = [x['TENANT_NAME'], x['TENANT_TYPE'], x['COMPATIBILITY_MODE'], x['PRIMARY_ZONE'],
            ...
            if show_protection:
                row.extend([x['PROTECTION_MODE'], x['PROTECTION_LEVEL']])
            return row

        stdio.print_list(tenant_infos, headers, tenant_basic_row, title='tenant basic info')
