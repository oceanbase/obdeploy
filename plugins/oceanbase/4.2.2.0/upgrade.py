...
class Upgrader(object):
    ...
    def bootstrap_plugin(self):
        if self._bootstrap_plugin is None:
            self._bootstrap_plugin = self.search_py_script_plugin(self.next_stage, 'bootstrap')
        return self._bootstrap_plugin
    ...
    def call_plugin(self, plugin, *args, **kwargs):
        return plugin(self.plugin_context.namespace, self.plugin_context.namespaces, self.plugin_context.deploy_name,
                      ...
                      self.plugin_context.stdio.sub_io(), *args, **kwargs)
    ...
    def connect(self):
        if self.cursor is None or self.execute_sql('show tables', error=False) is False:
            ...
            self.ocs_cursor = ret.get_return('ocs_cursor')
    ...
    def take_over(self):
        ...
        return self.connect() and self.call_plugin(self.bootstrap_plugin)
