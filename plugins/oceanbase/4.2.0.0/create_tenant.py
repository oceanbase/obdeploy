...
def create_tenant(plugin_context, cursor, create_tenant_options=None, relation_tenants={}, cluster_configs={}, primary_tenant_info={}, standbyro_password='', *args, **kwargs):
    ...
    if not tenant_exists:
        ...
        if memory_size < MIN_MEMORY:
            return error('memory must greater then %s' % format_size(MIN_MEMORY))
        ...
        if primary_tenant_info:
            if parse_size(primary_memory_size) < STANDBY_MIN_MEMORY:
            ...
            if parse_size(primary_memory_size) < STANDBY_WARN_MEMORY:
                stdio.warn('Primary tenant memory_size: {}B , suggestion: {}B'.format(primary_memory_size, STANDBY_WARN_MEMORY))
            if parse_size(primary_log_disk_size) < STANDBY_MIN_LOG_DISK_SIZE:
        ...
        if not primary_tenant_info:
            ...
            grant all on *.* to '{username}' WITH GRANT OPTION;""".format(
                    ...
                    # todo: fix oracle user create
                    sql = """create {username} IDENTIFIED BY {password};
        grant all on *.* to {username} WITH GRANT OPTION;
        grant dba to {username};
        grant all privileges to {username};"""
                if not exec_sql_in_tenant(sql=sql, cursor=cursor, tenant=name, mode=mode):
