...
def init(plugin_context, source_option=None, *args, **kwargs):
    ...
    stdio.start_loading('Initializes OMS mount path')
    ...
    for server in cluster_config.servers:
        ...
        if logs_path or run_path or store_path:
            if not (logs_path and run_path and store_path):
                global_ret = False
                stdio.error(EC_FAIL_TO_INIT_PATH.format(server=server, key='mount path', msg='logs_mount_path, run_mount_path and store_mount_path must all be configured'))
