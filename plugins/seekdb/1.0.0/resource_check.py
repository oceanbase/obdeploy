    ...
    for ip in servers_disk:
        ...
        if ret and IS_DARWIN:
            try:
                ...
                elif total_use > server_memory_stats['free'] + server_memory_stats['cached']:
                    for server in ip_servers:
                        ...
                        for key in ['memory_limit', 'memory_limit_percentage']:
                            if key in global_generate_config or key in server_generate_config:
                                ...
                                break
                        error(server, 'mem', err.EC_OBSERVER_NOT_ENOUGH_MEMORY_CACHED.format(ip=ip, free=str(Capacity(server_memory_stats['free'])), cached=str(Capacity(server_memory_stats['cached'])), need=str(Capacity(total_use))), [suggest])
                else:
                    system_memory_check()
            except Exception:
