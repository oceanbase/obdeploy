                            ...
                            for _, reserved_resource in resource.items():
                                if 'memory' in reserved_resource.keys():
                                    for server_ip in reserved_resource['memory'].keys():
                                        if server_ip == ip:
                                            available_memory = available_memory - Capacity(reserved_resource['memory'][server_ip]).bytes
                ...
                if MIN_NEED > disk_free:
                    ...
                    if auto_set_datafile_size:
                        ...
                        if not datafile_maxsize:
                            datafile_maxsize = disk_free - min_log_size - SLOG_SIZE - log_disk_size
                    if datafile_maxsize + log_disk_size + min_log_size + SLOG_SIZE > disk_free:
