...
def reload(plugin_context, new_cluster_config, *args, **kwargs):
    ...
    role = cursor.fetchone('select role from __all_virtual_server_stat;')['role']
    for key in global_change_conf:
        try:
            if key in set(need_restart_key):
                continue
            if key in ['proxyro_password', 'root_password'] and role != 'STANDBY':
            ...
            if key == 'enable_auto_start':
                if global_change_conf[key]['value'] == False:
                    ...
                    for server in cluster_config.servers:
                        ...
                        cluster_name = plugin_context.deploy_name
                        observer_service = 'obd_seekdb_%s.service' % cluster_name
