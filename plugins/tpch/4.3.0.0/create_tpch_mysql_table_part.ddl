...
drop table if exists lineitem;
    ...
    partition by key (l_orderkey) partitions cpu_num with column group (each column);
...
drop table if exists orders;
    ...
    tablegroup = tpch_tg_lineitem_order_group
    partition by key(o_orderkey) partitions cpu_num with column group (each column);
...
drop table if exists partsupp;
    ...
    partition by key(ps_partkey) partitions cpu_num with column group (each column);
...
drop table if exists part;
  ...
  tablegroup tpch_tg_partsupp_part
  partition by key(p_partkey) partitions cpu_num with column group (each column);
...
drop table if exists customer;
  ...
  partition by key(c_custkey) partitions cpu_num with column group (each column);
...
drop table if exists supplier;
  ...
  partition by key(s_suppkey) partitions cpu_num with column group (each column);
...
drop table if exists nation;
  ...
  primary key (n_nationkey))row_format = condensed with column group (each column);
...
drop table if exists region;
  ...
  primary key (r_regionkey))row_format = condensed with column group (each column);