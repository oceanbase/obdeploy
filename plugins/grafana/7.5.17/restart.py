    ...
    def restart(self):
        ...
        if self.new_cluster_config:
            if not self.connect(self.cluster_config):
                return False
            if not self.call_plugin(self.reload_plugin, clients=clients, cursor=self.cursors, new_cluster_config=self.new_cluster_config, repository_dir=self.repository.repository_dir):
                return False
        ...
        if not self.call_plugin(self.start_plugin, clients=clients, cluster_config=cluster_config, need_bootstrap=False):
            self.rollback()
            self.stdio.stop_loading('stop_loading', 'fail')
            return False
        if self.connect(cluster_config):
            return self.call_plugin(self.display_plugin, clients=clients, cluster_config=cluster_config, cursor=self.cursors) if self.display_plugin else True
        return False
