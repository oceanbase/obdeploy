...
def get_missing_required_parameters(parameters):
    ...
    for key in ["jdbc_url", "jdbc_password", "jdbc_username", "cluster_name", "ob_cluster_id", "root_sys_password",
                "server_addresses", "agent_username", "agent_password", "ocp_root_password"]:
...
def prepare_parameters(cluster_config, stdio):
    ...
    for comp in ["oceanbase", "oceanbase-ce"]:
        ...
        if comp in cluster_config.depends:
            ...
            ocp_meta_keys = [
                "ocp_meta_tenant", "ocp_meta_db", "ocp_meta_username", "ocp_meta_password", "appname", "cluster_id", "root_password", "ocp_root_password"
            ]
    ...
    for server in cluster_config.servers:
        ...
        if missed_keys:
            ...
            depends_key_maps = {
                ...
                "ocp_meta_tenant": "ocp_meta_tenant"
            }
...
def start(plugin_context, start_env=None, *args, **kwargs):
    ...
    exclude_keys = ["home_path", "port", "jdbc_url", "jdbc_username", "jdbc_password", "cluster_name", "ob_cluster_id",
                    "root_sys_password", "server_addresses", "agent_username", "agent_password", "memory_size", "ocp_root_password", "ocp_meta_tenant"]
    ...
    for server in cluster_config.servers:
        ...
        obCredentials = []
        if server_config.get('ocp_root_password'):
            obCredentials = [{
                "tenantName": server_config["ocp_meta_tenant"]["tenant_name"],
                "username": "root",
                "password": server_config['ocp_root_password'],
            }]
        data = {
            "cluster": {
                ...
                "obCredentials": obCredentials if not client.execute_command('ls %s' % bootstrap_flag) else []
            },
