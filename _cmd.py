...
#
# OceanBase Deploy is a free software: you can redistribute it and/or modify
...
class ObdCommand(BaseCommand):
    ...
    def do_command(self):
        ...
        # ROOT_IO.print('grep %s ~/.obd/log/obd' % trace_id)
        return ret
...
class ClusterTenantCreateCommand(ClusterMirrorCommand):
    ...
    def __init__(self):
        ...
        self.parser.add_option('--max-iops', type='int', help="Unit max iops number.", default=128)
        self.parser.add_option('--min-iops', type='int', help="Unit min iops number.")
        self.parser.add_option('--max-session-num', type='int', help="Unit max session number.", default=64)
        ...
        self.parser.add_option('--primary-zone', type='string', help="Tenant primary zone.", default='RANDOM')
        self.parser.add_option('--locality', type='string', help="Tenant locality.")
        self.parser.add_option('-s', '--variables', type='string', help="Set the variables for the system tenant.")
...
class MySQLTestCommand(TestMirrorCommand):
    ...
    def __init__(self):
        ...
        self.parser.add_option('--user', type='string', help='Username for a test.', default='admin')
        self.parser.add_option('--password', type='string', help='Password for a test.', default='admin')
        self.parser.add_option('--database', type='string', help='Database for a test.', default='test')
        ...
        self.parser.add_option('--log-dir', type='string', help='The log file directory.', default='./log')
        self.parser.add_option('--tmp-dir', type='string', help='Temporary directory for mysqltest.', default='./tmp')
        self.parser.add_option('--var-dir', type='string', help='Var directory to use when run mysqltest', default='./var')
...
class SysBenchCommand(TestMirrorCommand):
    ...
    def __init__(self):
        ...
        self.parser.add_option('--component', type='string', help='The component for mysqltest.')
        self.parser.add_option('--test-server', type='string', help='The server for mysqltest. By default, the first root server in the component is the mysqltest server.')
        self.parser.add_option('--user', type='string', help='Username for a test.', default='root')
        self.parser.add_option('--password', type='string', help='Password for a test.')
        self.parser.add_option('--tenant', type='string', help='Tenant for a test.', default='test')
        self.parser.add_option('--database', type='string', help='Database for a test.', default='test')
        self.parser.add_option('--obclient-bin', type='string', help='OBClient bin path.', default='obclient')
        self.parser.add_option('--sysbench-bin', type='string', help='Sysbench bin path.', default='sysbench')
        self.parser.add_option('--script-name', type='string', help='Sysbench lua script file name.', default='oltp_write_only.lua')
        self.parser.add_option('--sysbench-script-dir', type='string', help='The directory of the sysbench lua script file.', default='/usr/sysbench/share/sysbench')
        self.parser.add_option('--table-size', type='int', help='Number of data initialized per table.', default=100000)
        self.parser.add_option('--tables', type='int', help='Number of initialization tables.', default=32)
        self.parser.add_option('--threads', type='int', help='Number of threads to use.', default=150)
        self.parser.add_option('--time', type='int', help='Limit for total execution time in seconds.', default=60)
        self.parser.add_option('--interval', type='int', help='Periodically report intermediate statistics with a specified time interval in seconds. 0 disables intermediate reports.', default=10)
        ...
        self.parser.add_option('--skip_trx', type='string', help='Enable or disable a transaction in a read-only test. The default value is on.')
