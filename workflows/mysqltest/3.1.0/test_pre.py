...
def test_pre(plugin_context, workflow, repository, *args, **kwargs):
    ...
    workflow.add_with_component_version_kwargs(const.STAGE_SECOND, 'mysqltest', target_repository_version, {"repository": target_repository}, 'check_opt')
    if not env['init_only']:
        workflow.add_with_component_version_kwargs(const.STAGE_THIRD, 'mysqltest', target_repository_version, {"repository": target_repository}, 'check_test_pre')
        workflow.add_with_component_version_kwargs(const.STAGE_THIRD, 'mysqltest', target_repository_version, {"repository": target_repository}, 'check_test')

    if env['need_init'] or env['init_only']:
        workflow.add_with_component_version_kwargs(const.STAGE_FOURTH, 'mysqltest', target_repository_version, {"repository": target_repository, **env}, 'init_pre')
        workflow.add_with_component_version_kwargs(const.STAGE_FOURTH, 'mysqltest', target_repository_version, {"repository": target_repository, **env}, 'init')
        if fast_reboot:
            ...
            workflow.add_with_component_version_kwargs(const.STAGE_SEVENTH, 'mysqltest', target_repository_version, {"repository": target_repository}, 'init')
