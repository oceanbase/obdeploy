...
def start(plugin_context, workflow, *args, **kwargs):
    ...
    workflow.add(const.STAGE_FIRST, 'gen_ca', 'start_pre', 'start', 'health_check', 'connect', 'bootstrap', 'seekdb_standby_detect', 'obshell_start', 'obshell_bootstrap')
    ...
    workflow.add(const.STAGE_SECOND, 'dump_relation', 'sync_cluster_to_obshell', 'check_tls')
    ...
    if enable_observer_auto and not client.execute_command(cmd):
        workflow.add(const.STAGE_SECOND, 'generate_conf_and_systemd', 'enable_auto_start')
