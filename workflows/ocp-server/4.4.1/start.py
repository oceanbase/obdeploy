...
def start(plugin_context, workflow, *args, **kwargs):
    ...
    if not(source_option == 'upgrade' or client.execute_command(cmd)):
        workflow.add_with_component_version_kwargs(const.STAGE_FIRST, const.COMP_OB_AI if const.COMP_OB_AI in repository_name else (const.COMP_OB_CE if const.COMP_OB_CE in repository_name else const.COMP_OB), '4.0.0.0', {'scale_out_component': const.COMP_OCP_SERVER_CE if const.COMP_OCP_SERVER_CE in repository_name else const.COMP_OCP_SERVER}, 'connect', 'create_tenant', 'create_user', 'import_time_zone')
    workflow.add(const.STAGE_FIRST, 'extract_doc', 'start', 'health_check', 'bootstrap', 'connect', 'upload_packages')
