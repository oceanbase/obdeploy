...
def restart(plugin_context, workflow, *args, **kwargs):
    ...
    workflow.add_with_component(const.STAGE_FIRST, 'general', 'stop_container')
    if new_clients:
        workflow.add_with_component_version_kwargs(const.STAGE_FIRST, 'general', '0.1', {"new_clients": new_clients}, 'chown_dir')
    workflow.add_with_kwargs(const.STAGE_FIRST, {"clients": new_clients if new_clients else clients, "new_cluster_config": new_cluster_config, "cluster_config": new_cluster_config if new_cluster_config else cluster_config}, 'generate_oms_config', 'distribute_config', 'start', 'health_check', 'reload')
    ...
    workflow.add_with_kwargs(const.STAGE_FIRST, {"clients": clients, "cluster_config": cluster_config}, 'display')
