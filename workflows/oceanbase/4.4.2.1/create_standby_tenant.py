...
def create_standby_tenant(plugin_context, workflow, *args, **kwargs):
    ...
    if getattr(options, 'type') == const.SERVICE_MODE:
        workflow.add(const.STAGE_THIRD, 'create_standby_tenant_pre', 'set_tenant_sync_mode_pre', 'create_standby_tenant')
        workflow.add(const.STAGE_THIRD, 'dump_standby_relation', 'set_tenant_sync_mode')
    else:
