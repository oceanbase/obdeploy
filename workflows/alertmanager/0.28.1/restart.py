...
def restart(plugin_context, workflow, *args, **kwargs):
    ...
    workflow.add_with_kwargs(const.STAGE_FIRST, {'cursor': cursor if cursor else None, 'need_bootstrap': need_bootstrap,
                                                 ...
                                                 'cluster_config': new_cluster_config if new_cluster_config else cluster_config}, 'parameter_pre', 'start_pre', 'start', 'health_check')
    if const.COMP_PROMETHEUS in cluster_config.depends:
        for repository in plugin_context.repositories:
            if repository.name == const.COMP_PROMETHEUS:
                workflow.add_with_component_version_kwargs(const.STAGE_FIRST, 'prometheus', repository.version, {"repository": repository}, 'connect')
                workflow.add_with_kwargs(const.STAGE_FIRST, {'new_cluster_config': new_cluster_config, 'cluster_config': new_cluster_config if new_cluster_config else cluster_config}, 'update_prometheus')
