...
def init(plugin_context, workflow, *args, **kwargs):
    ...
    workflow.add_with_component_version_kwargs(const.STAGE_FIRST, 'general', '0.1', {'docker_compose_check': True}, 'docker_check')
    if not getattr(options, 'skip_cluster_status_check', False):
        workflow.add(const.STAGE_SECOND, 'status')
        workflow.add_with_component_version_kwargs(const.STAGE_SECOND, 'general', '0.1', {'target_status': ClusterStatus.STATUS_STOPPED}, 'status_check')
