...
def restart(plugin_context, workflow, *args, **kwargs):
    ...
    workflow.add(const.STAGE_FIRST,  'parameter_pre', 'generate_env_file', 'get_services_status', 'stop')
    if new_clients:
        workflow.add_with_component_version_kwargs(const.STAGE_FIRST, 'general', '0.1', {"new_clients": new_clients}, 'chown_dir')
    workflow.add_with_kwargs(const.STAGE_FIRST, {"clients": new_clients if new_clients else clients, "new_cluster_config": new_cluster_config, "cluster_config": new_cluster_config if new_cluster_config else cluster_config}, 'get_services_status', 'start')
